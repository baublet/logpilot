...
// /home/baublet/logpilot/node_modules/.pnpm/ansi-to-html@0.7.2/node_modules/entities/lib/index.js
...
// /home/baublet/logpilot/node_modules/ansi-to-html/lib/ansi_to_html.js
var require_ansi_to_html = __commonJS((exports, module) => {
  ...
  var setStyleColor = function(red, green, blue, colors) {
  ...
  var toColorHexString = function(ref) {
    var results = [];
  ...
  var generateOutput = function(stack, token, data, options) {
    var result;
    if (token === "text") {
  ...
  var handleRgb = function(stack, data) {
  ...
  var handleDisplay = function(stack, code, options) {
  ...
  var resetStyles = function(stack) {
  ...
  var notCategory = function(category) {
    return function(e) {
      return (category === null || e.category !== category) && category !== "all";
    };
  };
  var categoryForCode = function(code) {
    code = parseInt(code, 10);
    var result = null;
    if (code === 0) {
      result = "all";
  ...
  var pushText = function(text, options) {
    if (options.escapeXML) {
      return entities.encodeXML(text);
    }
    return text;
  };
  var pushTag = function(stack, tag, style) {
    if (!style) {
      style = "";
    }
    stack.push(tag);
  ...
  var pushStyle = function(stack, style) {
    return pushTag(stack, "span", style);
  };
  var pushForegroundColor = function(stack, color) {
    return pushTag(stack, "span", "color:" + color);
  };
  var pushBackgroundColor = function(stack, color) {
    return pushTag(stack, "span", "background-color:" + color);
  };
  var closeTag = function(stack, style) {
    var last;
  ...
  var tokenize = function(text, options, callback) {
    var ansiMatch = false;
    var ansiHandler = 3;
    function remove() {
      return "";
  ...
  var updateStickyStack = function(stickyStack, token, data) {
    if (token !== "text") {
...
// src/types.ts
function serializeClientWorkerMessage(message) {
...
var isRegExp = function(query) {
  return /\/(.*)\/(.*)/.test(query);
};
var connectionRetryTimeout = 0;
...
var websocketUrl = (origin + "/web-socket" + search).replace(/^http/, "ws");
...
socket.onclose = () => {
  connectionRetryTimeout += connectionRetryTimeout === 0 ? 100 : connectionRetryTimeout * 2;
  console.log(`Disconnected from server. Trying again in ${connectionRetryTimeout}ms`);
  ...
  if (parsedMessage.type === "get-filtered-lines") {
    ...
    const searchInstance = new SearchInstance({
      ...
      onResults: (results) => {
        ...
        self.postMessage(serializeClientWorkerMessage({
...
var logStore = {
  ...
  appendLogs: (logs) => {
    ...
    self.postMessage(serializeClientWorkerMessage({ type: "append-logs", logs, htmlLogs }));
  }
};

class SearchInstance {
  static SEARCH_BATCH_SIZE = 1e4;
  _query;
  _queryRegex;
  _timer;
  _onResults;
